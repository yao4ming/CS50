...
char* indexes(const char* path)
{
    const int indexExtension = 12;  // - longest extension /index.html plus NULL terminator
    char* indexPath = malloc(strlen(path) + indexExtension);
    strncpy(indexPath, path, strlen(path));
    indexPath[strlen(path)] = '\0';
    strncat(indexPath, "index.php", 10);
    printf("indexpath - %s\n", indexPath);
    if (access(indexPath, F_OK) == 0) return indexPath;
    
    strncpy(indexPath, path, strlen(path));
    indexPath[strlen(path)] = '\0';
    strncat(indexPath, "index.html", 11);
    printf("indexpath - %s\n", indexPath);
    if (access(indexPath, F_OK) == 0) return indexPath;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    for (int c = fgetc(file); c != EOF; c = fgetc(file)) {
        buffer[i] = (char) c;
        i++;
        buffer = (char*) realloc(buffer, sizeof(BYTE) + i); 
    }
...
{
    printf("PATH - %s\n", path);
    char* extension = malloc(sizeof(char) * strlen(path));
    int indexBeforeExtension = (int)(strchr(path, '.') - path);
    int i = 0;
    for (int beg = indexBeforeExtension; beg < strlen(path); beg++, i++) {
        extension[i] = path[beg];
    }
    extension[i] = '\0';

    if (strncasecmp(extension, ".css", 4) == 0) {
        return "text/css";
    } else if (strncasecmp(extension, ".ico", 4) == 0) {
        return "image/x-icon";
    } else if (strncasecmp(extension, ".html", 5) == 0) {
        return "text/html";
    } else if (strncasecmp(extension, ".gif", 4) == 0) {
        return "image/gif";
    } else if (strncasecmp(extension, ".jpg", 4) == 0) {
        return "image/jpeg";
    } else if (strncasecmp(extension, ".css", 4) == 0) {
        return "text/css";
    } else if (strncasecmp(extension, ".js", 3) == 0) {
        return "text/javascript";
    } else if (strncasecmp(extension, ".php", 4) == 0) {
        return "text/x-php";
    } else if (strncasecmp(extension, ".png", 4) == 0) {
        return "image/png";
    } else {
        return NULL;
    }
...
{
    //clear abs_path and query
    for (int i = 0; i <= LimitRequestLine; i++) {
        abs_path[i] = query[i] = '\0';
    }
    //METHOD
    int indexAfterMethod = strchr(line, ' ') == NULL ? -1 : (int)(strchr(line, ' ') - line);
    if (indexAfterMethod < 0) {
        printf("Invalid method index");
        return false;
    }
    char method[indexAfterMethod];
    strncpy(method, line, indexAfterMethod);
    printf("Method - %s\n", method);
    ...
    //REQUEST TARGET
    bool hasQuery = strchr(line + indexAfterMethod+1, '?') != NULL;
    int indexAfterRequestTarget = hasQuery ?
                                  (int)(strchr(line + indexAfterMethod+1, '?') - line) :
                                  (int)(strchr(line + indexAfterMethod+1, ' ') - line);
    if (indexAfterRequestTarget < 0) {
        printf("Invalid request target index");
        return false;
    }
    strncpy(abs_path, line + indexAfterMethod + 1, indexAfterRequestTarget - 1 - indexAfterMethod);
    printf("Absolute Path - %s\n", abs_path);
    if (abs_path[0] != '/') {
        printf("/ must be first");
        error(501);
        return false;
    }

    //QUERY
    int indexAfterQuery = 0;
    if (hasQuery) {
        indexAfterQuery = (int)(strchr(line + indexAfterRequestTarget, ' ') - line);
        strncpy(query, line + indexAfterRequestTarget + 1,  indexAfterQuery - 1 - indexAfterRequestTarget);
    ...
    //HTTP-VERSION
    char httpVersion[strlen(line)];
    hasQuery ? strncpy(httpVersion, line + indexAfterQuery + 1, strlen(line) - indexAfterQuery) :
                strncpy(httpVersion, line + indexAfterRequestTarget + 1, strlen(line) - indexAfterRequestTarget);
    httpVersion[strlen(httpVersion)-2] = '\0';
    printf("HTTP VERSION - %s\n", httpVersion);
    if (strncmp(httpVersion, "HTTP/1.1", strlen(httpVersion)) != 0) {
        error(505);
        return false;
    }
